	...
	class deliveryModel extends Model
	{
		...
		public function getAllDelivery()
		{
			try
			{
				...
				$opt = array(
					'resource' 		=> 'deliveries',
					'output_format'	=> 'JSON',
					'display' 		=> '[id, id_carrier, id_range_price, id_zone, price]',
				);

				$result = $webService->get($opt);

				$resource = $result->deliveries->children();
			} 
		...
		public function addNewDelivery()
		{
			...
			try
			{
				...
				$opt = array('resource' => 'deliveries');

				if (isset($_GET['Create']))
					$xml = $webService->get(array('url' => PS_SHOP_PATH.'/api/deliveries?schema=blank'));
				else
		...
		public function deleteDelivery()
		{
			if (isset($_GET['DeleteID']))
			{
				...
				echo '<h1>Delivery Deletion</h1><br>';
				...
				try
				{
					...
					$webService->delete(array('resource' => 'deliveries', 'id' => intval($_GET['DeleteID'])));
