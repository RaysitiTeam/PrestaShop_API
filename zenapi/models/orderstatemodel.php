<?php	
	...
	class orderStateModel extends Model
	{
		...
		public function getAllOrderState()
		{
			try
			{
				...
				$opt = array(
					...
					'output_format'	=> 'JSON',
					'display' 		=> '[id, unremovable, delivery, send_email, invoice, shipped, paid, pdf_invoice, name]',
				);

				$result = $webService->get($opt);

				$resource = $result->order_states->children();
			} 
		...
		// Add new OrderState
		public function addNewOrderState()
		{
			...
			try
			{
				...
				if (isset($_GET['Create']))
					$xml = $webService->get(array('url' => PS_SHOP_PATH.'/api/order_states?schema=blank'));
				else
		...
		public function updateOrderState()
		{
			...
			try
			{
				...
				// Here we get the elements from children of order State markup which is children of prestashop root markup
				$resources = $xml->children()->children();
			}
			catch (PrestaShopWebserviceException $e)
			{
			...
			// We set a link to go back to list if we are in Order State's details
			if (isset($_GET['id']))
				echo '<a href="?">Return to the list</a>';
		...
		public function deleteOrderState()
		{
			if (isset($_GET['DeleteID']))
			{
				...
				echo '<h1>Order State Deletion</h1><br>';
				...
				try
				{
					...
					$webService->delete(array('resource' => 'order_states', 'id' => intval($_GET['DeleteID'])));
