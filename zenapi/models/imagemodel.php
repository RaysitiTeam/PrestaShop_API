	...
	class imageModel extends Model
	{
		...
		public function getAllImage()
		{
			try
			{
				...
				$resource = $result->images->children();
			} 
		...
		public function addNewImage()
		{
			...
			try
			{
				...
				if (isset($_GET['Create']))
					$xml = $webService->get(array('url' => PS_SHOP_PATH.'/api/images?schema=blank'));
				else
		...
		public function updateImage()
		{
			...
			try
			{
				...
				// Here we get the elements from children of image markup which is children of prestashop root markup
				$resources = $xml->children()->children();
			}
			catch (PrestaShopWebserviceException $e)
			{
			...
			// We set a link to go back to list if we are in image's details
			if (isset($_GET['id']))
				echo '<a href="?">Return to the list</a>';
		...
		public function deleteImage()
		{
			if (isset($_GET['DeleteID']))
			{
				...
				try
				{
					...
					$webService->delete(array('resource' => 'images', 'id' => intval($_GET['DeleteID'])));
