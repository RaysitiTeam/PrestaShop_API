<?php	
	...
	class orderDetailModel extends Model
	{
		...
		public function getAllOrderDetail()
		{
			try
			{
				...
				$opt = array(
					...
					'output_format'	=> 'JSON',
					'display' 		=> '[id, id_order, product_id, id_order_invoice, id_shop, product_name, product_quantity, product_quantity_in_stock, product_price, product_reference, original_product_price, total_price_tax_incl, total_price_tax_excl]',
				);

				$result = $webService->get($opt);

				$resource = $result->order_details->children();
			} 
		...
		// Add new OrderDetail
		public function addNewOrderDetail()
		{
			...
			try
			{
				...
				if (isset($_GET['Create']))
					$xml = $webService->get(array('url' => PS_SHOP_PATH.'/api/order_details?schema=blank'));
				else
		...
		public function updateOrderDetail()
		{
			...
			try
			{
				...
				// Here we get the elements from children of order detail markup which is children of prestashop root markup
				$resources = $xml->children()->children();
			}
			catch (PrestaShopWebserviceException $e)
			{
			...
			// We set a link to go back to list if we are in Order Detail's details
			if (isset($_GET['id']))
				echo '<a href="?">Return to the list</a>';
		...
		public function deleteOrderDetail()
		{
			if (isset($_GET['DeleteID']))
			{
				...
				echo '<h1>Order Detail Deletion</h1><br>';
				...
				try
				{
					...
					$webService->delete(array('resource' => 'order_details', 'id' => intval($_GET['DeleteID'])));
