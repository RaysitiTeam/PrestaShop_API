	...
	class countryModel extends Model
	{
		...
		public function getAllCountry()
		{
			...
			try
			{
				...
				// Option array of webservice to get country resource
				...
				// Get the elements form children of countries markup 'country'
				$resource = $result->countries->children();
			} 
		...
		{
			// Here we use the WebService to get the schema of "countries" resource
			try
			{
				$webService = new PrestaShopWebservice(PS_SHOP_PATH, PS_WS_AUTH_KEY, DEBUG);
		...
		{
			// First : We always get the country's list or a specific one
			try
			{
				$webService = new PrestaShopWebservice(PS_SHOP_PATH, PS_WS_AUTH_KEY, DEBUG);
		...
		public function deleteCountry()
		{
			if (isset($_GET['DeleteID']))
			{
				...
				echo '<h1>Country Deletion</h1><br>';
				...
				try
				{
					...
					$webService->delete(array('resource' => 'countries', 'id' => intval($_GET['DeleteID'])));
