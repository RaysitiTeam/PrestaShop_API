	...
	class invoiceModel extends Model
	{
		...
		public function getAllInvoice()
		{
			try
			{
				...
				$opt = array(
					...
					'output_format'	=> 'JSON',
					'display' 		=> '[id, id_order, number, delivery_number, delivery_date, total_discount_tax_excl, total_paid_tax_excl, total_products, shop_address]',
				);

				$result = $webService->get($opt);

				$resource = $result->order_invoices->children();
			} 
		...
		public function addNewInvoice()
		{
			...
			try
			{
				...
				$opt = array('resource' => 'order_invoices');

				if (isset($_GET['Create']))
					$xml = $webService->get(array('url' => PS_SHOP_PATH.'/api/order_invoices?schema=blank'));
				else
		...
		public function updateInvoice()
		{
			...
			try
			{
				...
				// Here we get the elements from children of invoice markup which is children of prestashop root markup
				$resources = $xml->children()->children();
			}
			catch (PrestaShopWebserviceException $e)
			{
			...
			// We set a link to go back to list if we are in invoice's details
			if (isset($_GET['id']))
				echo '<a href="?">Return to the list</a>';
		...
		public function deleteInvoice()
		{
			if (isset($_GET['DeleteID']))
			{
				...
				echo '<h1>Invoice Deletion</h1><br>';
				...
				try
				{
					...
					$webService->delete(array('resource' => 'order_invoices', 'id' => intval($_GET['DeleteID'])));
