	...
	class taxModel extends Model
	{
		...
		public function getAllTax()
		{
			...
			try
			{
				...
				// Option array of webservice to get tax resource
				$opt['resource'] = 'taxes';

				$opt = array(
					'resource' 		=> 'taxes',
					'output_format'	=> 'JSON',
					'display' 		=> '[id, rate, active, name]',
				);
		...
		{
			// Here we use the WebService to get the schema of "taxes" resource
			try
			{
				$webService = new PrestaShopWebservice(PS_SHOP_PATH, PS_WS_AUTH_KEY, DEBUG);
				...
				if (isset($_GET['Create']))
					$xml = $webService->get(array('url' => PS_SHOP_PATH.'/api/taxes?schema=blank'));
				else
		...
		{
			// First : We always get the Tax's list or a specific one
			try
			{
				$webService = new PrestaShopWebservice(PS_SHOP_PATH, PS_WS_AUTH_KEY, DEBUG);
		...
		public function deleteTax()
		{
			if (isset($_GET['DeleteID']))
			{
				...
				try
				{
					...
					$webService->delete(array('resource' => 'taxes', 'id' => intval($_GET['DeleteID'])));
