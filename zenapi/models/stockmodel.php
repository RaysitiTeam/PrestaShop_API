	...
	class stockModel extends Model
	{
		...
		public function getAllStock()
		{
			...
			try
			{
				...
				// Option array of webservice to get stock resource
				$opt['resource'] = 'stocks';
				...
				// Get the elements form children of stocks markup 'stock'
				$resource = $result->stocks->children();
			} 
		...
		{
			// Here we use the WebService to get the schema of "stocks" resource
			try
			{
				$webService = new PrestaShopWebservice(PS_SHOP_PATH, PS_WS_AUTH_KEY, DEBUG);

				$opt = array('resource' => 'stocks');

				if (isset($_GET['Create']))
					$xml = $webService->get(array('url' => PS_SHOP_PATH.'/api/stocks?schema=blank'));
				else
		...
		{
			// First : We always get the Stock's list or a specific one
			try
			{
				$webService = new PrestaShopWebservice(PS_SHOP_PATH, PS_WS_AUTH_KEY, DEBUG);
		...
		public function deleteStock()
		{
			if (isset($_GET['DeleteID']))
			{
				...
				try
				{
					...
					$webService->delete(array('resource' => 'stocks', 'id' => intval($_GET['DeleteID'])));
