<?php	
	...
	class guestModel extends Model
	{
		...
		public function getAllGuest()
		{
			...
			try
			{
				...
				$opt = array(
					'resource' 		=> 'guests',
					'output_format'	=> 'JSON',
					'display' 		=> '[id, id_customer, id_operating_system, screen_resolution_x, screen_resolution_y, accept_language]',
				);

				$result = $webService->get($opt);

				// Get the elements form children of guests markup 'guest'
				$resource = $result->guests->children();
			} 
		...
		{
			// Here we use the WebService to get the schema of "guests" resource
			try
			{
				$webService = new PrestaShopWebservice(PS_SHOP_PATH, PS_WS_AUTH_KEY, DEBUG);

				$opt = array('resource' => 'guests');
		...
		{
			// First : We always get the guest's list or a specific one
			try
			{
				$webService = new PrestaShopWebservice(PS_SHOP_PATH, PS_WS_AUTH_KEY, DEBUG);
			...
			// We set a link to go back to list if we are in guest's details
			if (isset($_GET['id']))
				echo '<a href="?">Return to the list</a>';
		...
		public function deleteGuest()
		{
			if (isset($_GET['DeleteID']))
			{
				...
				try
				{
					...
					$webService->delete(array('resource' => 'guests', 'id' => intval($_GET['DeleteID'])));
