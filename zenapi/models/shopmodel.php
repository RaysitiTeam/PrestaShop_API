	...
	class shopModel extends Model
	{
		...
		public function getAllShop()
		{
			try
			{
				...
				$opt = array(
					'resource' 		=> 'shops',
					'output_format'	=> 'JSON',
					'display' 		=> '[id, name, id_shop_group, id_category, id_theme, active]',
				);

				$result = $webService->get($opt);

				$resource = $result->shops->children();
			} 
		...
		public function addNewShop()
		{
			...
			try
			{
				...
				$opt = array('resource' => 'shops');

				if (isset($_GET['Create']))
					$xml = $webService->get(array('url' => PS_SHOP_PATH.'/api/shops?schema=blank'));
				else
		...
		public function deleteShop()
		{
			if (isset($_GET['DeleteID']))
			{
				...
				try
				{
					...
					$webService->delete(array('resource' => 'shops', 'id' => intval($_GET['DeleteID'])));
