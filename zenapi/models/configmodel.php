	...
	class configModel extends Model
	{
		...
		public function getAllConfig()
		{
			...
			try
			{
				...
				// Option array of webservice to get config resource
				$opt['resource'] = 'configurations';

				$opt = array(
					'resource' 		=> 'configurations',
					'output_format'	=> 'JSON',
					'display' 		=> '[id, value, name,date_add, date_upd]',
				);

				$result = $webService->get($opt);

				// Get the elements form children of configurations markup 'config'
				$resource = $result->configurations->children();
			} 
		...
		{
			// Here we use the WebService to get the schema of "configurations" resource
			try
			{
				$webService = new PrestaShopWebservice(PS_SHOP_PATH, PS_WS_AUTH_KEY, DEBUG);
		...
		public function updateConfig()
		{
			...
			// We set a link to go back to list if we are in config's details
			if (isset($_GET['id']))
				echo '<a href="?">Return to the list</a>';
		...
		public function deleteConfig()
		{
			if (isset($_GET['DeleteID']))
			{
				...
				echo '<h1>Configuration Deletion</h1><br>';
				...
				try
				{
					...
					$webService->delete(array('resource' => 'configurations', 'id' => intval($_GET['DeleteID'])));
