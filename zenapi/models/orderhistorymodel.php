<?php	
	...
	class orderHistoryModel extends Model
	{
		...
		public function getAllOrderHistory()
		{
			try
			{
				...
				$opt = array(
					...
					'output_format'	=> 'JSON',
					'display' 		=> '[id, id_employee, id_order_state, id_order, date_add]',
				);

				$result = $webService->get($opt);

				$resource = $result->order_histories->children();
			} 
		...
		// Add new OrderHistory
		public function addNewOrderHistory()
		{
		...
		public function updateOrderHistory()
		{
			...
			try
			{
				...
				// Here we get the elements from children of order History markup which is children of prestashop root markup
				$resources = $xml->children()->children();
			}
			catch (PrestaShopWebserviceException $e)
			{
			...
			// We set a link to go back to list if we are in Order History's details
			if (isset($_GET['id']))
				echo '<a href="?">Return to the list</a>';
		...
		public function deleteOrderHistory()
		{
			if (isset($_GET['DeleteID']))
			{
				...
				echo '<h1>Order History Deletion</h1><br>';
				...
				try
				{
					...
					$webService->delete(array('resource' => 'order_histories', 'id' => intval($_GET['DeleteID'])));
