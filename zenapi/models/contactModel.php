	...
	class contactModel extends Model
	{
		...
		public function getAllContact()
		{
			...
			try
			{
				...
				// Option array of webservice to get contact resource
				$opt['resource'] = 'contacts';

				$opt = array(
					'resource' 		=> 'contacts',
					'output_format'	=> 'JSON',
					'display' 		=> '[id, email, name, description]',
				);

				$result = $webService->get($opt);

				// Get the elements form children of contacts markup 'address'
				$resource = $result->contacts->children();
			} 
		...
		{
			// Here we use the WebService to get the schema of "contacts" resource
			try
			{
				$webService = new PrestaShopWebservice(PS_SHOP_PATH, PS_WS_AUTH_KEY, DEBUG);

				$opt = array('resource' => 'contacts');
		...
		{
			// First : We always get the contact's list or a specific one
			try
			{
				$webService = new PrestaShopWebservice(PS_SHOP_PATH, PS_WS_AUTH_KEY, DEBUG);
			...
			// We set a link to go back to list if we are in contact's details
			if (isset($_GET['id']))
				echo '<a href="?">Return to the list</a>';
		...
		public function deleteContact()
		{
			if (isset($_GET['DeleteID']))
			{
				...
				echo '<h1>Contact Deletion</h1><br>';
				...
				try
				{
					...
					$webService->delete(array('resource' => 'contacts', 'id' => intval($_GET['DeleteID'])));
