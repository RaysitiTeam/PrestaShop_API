	...
	class productSupplierModel extends Model
	{
		...
		public function getAllProductSupplier()
		{
			try
			{
				...
				$opt = array(
					...
					'output_format'	=> 'JSON',
					'display' 		=> '[id, id_product, id_supplier, id_currency, product_supplier_price_te]',
				);

				$result = $webService->get($opt);

				$resource = $result->product_suppliers->children();
			} 
		...
		public function addNewProductSupplier()
		{
			...
			try
			{
				...
				if (isset($_GET['Create']))
					$xml = $webService->get(array('url' => PS_SHOP_PATH.'/api/product_suppliers?schema=blank'));
				else
		...
		public function updateProductSupplier()
		{
			...
			try
			{
				...
				// Here we get the elements from children of Product Supplier markup which is children of prestashop root markup
				$resources = $xml->children()->children();
			}
			catch (PrestaShopWebserviceException $e)
			{
			...
			// We set a link to go back to list if we are in Product Supplier's details
			if (isset($_GET['id']))
				echo '<a href="?">Return to the list</a>';
		...
		public function deleteProductSupplier()
		{
			if (isset($_GET['DeleteID']))
			{
				...
				echo '<h1>Product Supplier Deletion</h1><br>';
				...
				try
				{
					...
					$webService->delete(array('resource' => 'product_suppliers', 'id' => intval($_GET['DeleteID'])));
			...
			{
				// Else get Product Supplier list
				try
				{
					$webService = new PrestaShopWebservice(PS_SHOP_PATH, PS_WS_AUTH_KEY, DEBUG);
					$opt = array('resource' => 'product_suppliers');
				...
				echo '<h1>Product Supplier List</h1>';
				echo '<table border="5">';
				if (isset($resources))
				{
					echo '<tr>';
